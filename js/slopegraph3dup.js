...
d3.csv("data/gi_10_13.csv", function(data) {
	sgmdata = data;
	//schaal voor y-as
	/*var yScale = d3.scale.linear()
					.domain([d3.min(sgmdata, function(d) { return d["GI2010"]; }), d3.max(sgmdata, function(d) { return d["GI2010"]; })])
					.range([h-margin, margin + margintop]);*/
	
	var yScale = d3.scale.linear()
					.domain([d3.min(sgmdata, function(d) { return 84; }), d3.max(sgmdata, function(d) { return 7013; })])
					.range([h-margin, margin + margintop]);
	
	//teken bolletjes voor 2010
	var circles10 = svg3.selectAll("circle")
		.data(sgmdata)
		.enter()
		.append("circle");
	
	circles10.attr("cx",xfirst)
		.attr("cy", function(d) {return yScale(d["GI2010"]);})
		.attr("r", "3")
		.attr("class", "2010")
		...
		.attr("fill-opacity", "1")
		.append("svg:title")
        .text(function(d) {return d["Chain"] + ": " + d["GI2010"]  + "%"; });
	
	//teken bolletjes voor 2013
	var circles13 = svg3.selectAll("circle.y2013")
		.data(sgmdata)
		.enter()
		.append("circle");
	
	circles13.attr("cx",xsecond)
		.attr("cy", function(d) {return yScale(d["GI2013"]);})
		.attr("class", "2013")
		.attr("r", "3")
		.attr("class", function(d) {return d["Region"] + " " + d["Commodity"] + " " + d["Country"]})
		.attr("fill", "#e3e3e3")
		.attr("fill-opacity", "1")
		.append("svg:title")
        .text(function(d) {return d["Chain"] + ": " + d["GI2013"] + "%"; });
	...
	var labels = svg3.selectAll("text")
		...
		.enter()
		.append("text")
		.text(function(d) {return d["GI2013"] + " $   " + d["Chain"]; })
		.attr("class", function(d) {return d["Region"] + " " + d["Commodity" ] + " " + d["Country"] + " label13"})
		.attr("x", xsecond + 5)
		.attr("y", function(d) {return yScale(d["GI2013"]) + 4;})
		.attr("fill", "#D9534F")
	...
	lines.attr("x1", xfirst)
		.attr("x2", xsecond)
		.attr("y1", function(d) {return yScale(d["GI2010"]); })
		.attr("y2", function(d) {return yScale(d["GI2013"]); })
		...
		.append("svg:title")
        .text(function(d) {return d["Chain"] + ": from " + d["GI2010"] +" to " + d["GI2013"] + " $"; });
	...
	d3.selectAll("#textdata").selectAll("div")
		...
		.html(function(d) {return "<h3>" + d["Commodity"] + " " + d["Country"] + "</h3><p>" + d["Chain"] + "</p><p>From " + d["GI2010"] +" $ in 2010 to " + d["GI2013"] + " $ in 2013</p>"; })
		.style("visibility", "hidden");
