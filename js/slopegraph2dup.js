...
d3.csv("data/ciy_10_13.csv", function(data) {
	sgmdata = data;
	//schaal voor y-as
	var yScale = d3.scale.linear()
					.domain([d3.min(sgmdata, function(d) { return 5.4; }), d3.max(sgmdata, function(d) { return d["CIY2013"]; })])
					.range([h-margin, margin + margintop]);
	
	//teken bolletjes voor 2010
	var circles10 = svg2.selectAll("circle")
		.data(sgmdata)
		.enter()
		.append("circle");
	
	circles10.attr("cx",xfirst)
		.attr("cy", function(d) {return yScale(d["CIY2010"]);})
		.attr("r", "3")
		.attr("class", "2010")
		...
		.attr("fill-opacity", "1")
		.append("svg:title")
        .text(function(d) {return d["Chain"] + ": " + d["CIY2010"]  + "%"; });
	
	//teken bolletjes voor 2013
	var circles13 = svg2.selectAll("circle.y2013")
		.data(sgmdata)
		.enter()
		.append("circle");
	
	circles13.attr("cx",xsecond)
		.attr("cy", function(d) {return yScale(d["CIY2013"]);})
		.attr("class", "2013")
		.attr("r", "3")
		.attr("class", function(d) {return d["Region"] + " " + d["Commodity"] + " " + d["Country"]})
		.attr("fill", "#e3e3e3")
		.attr("fill-opacity", "1")
		.append("svg:title")
        .text(function(d) {return d["Chain"] + ": " + d["CIY2013"] + "%"; });
	...
	var labels = svg2.selectAll("text")
		...
		.enter()
		.append("text")
		.text(function(d) {return d["CIY2013"] + "%   " + d["Chain"]; })
		.attr("class", function(d) {return d["Region"] + " " + d["Commodity" ] + " " + d["Country"] + " label13"})
		.attr("x", xsecond + 5)
		.attr("y", function(d) {return yScale(d["CIY2013"]) + 4;})
		.attr("fill", "#D9534F")
	...
	lines.attr("x1", xfirst)
		.attr("x2", xsecond)
		.attr("y1", function(d) {return yScale(d["CIY2010"]); })
		.attr("y2", function(d) {return yScale(d["CIY2013"]); })
		...
		.append("svg:title")
        .text(function(d) {return d["Chain"] + ": from " + d["CIY2010"] +" to " + d["CIY2013"] + "%"; });
	...
	d3.selectAll("#textdata").selectAll("div")
		...
		.html(function(d) {return "<h3>" + d["Commodity"] + " " + d["Country"] + "</h3><p>" + d["Chain"] + "</p><p>From " + d["CIY2010"] +"% in 2010 to " + d["CIY2013"] + "% in 2013</p>"; })
		.style("visibility", "hidden");
