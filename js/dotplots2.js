...
function movebubbels() {
	...
	d3.select("svg").selectAll("circle")
		.transition()
				.duration(3000)
				.attr("cy", function(d) {return 450 - val2xcoord10(d[indyear]) + (Math.random() - 0.5) * 50 });
...
function changetoind(indicator) {
	...
	d3.selectAll(".datatext")
		.html(function(d) {return "<h3>" + d["Name"] + "</h3><p>" + d["Chain"] + "</p><p>From " + d[indsel + "10"] + " in 2010 to " + d[indsel + "13"] + " in 2013</p>"; });
}
